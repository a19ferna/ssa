...
class Wasserstein():
    ...
    def transform(self, y_test, x_ssa_test):
        ...
        if len(set(x_ssa_test)) > 2:
            raise ValueError(
                'x_ssa_test should have at most 2 modalities')

        if all(elem in list(self.sens_val_0, self.sens_val_1) for elem in list(set(x_ssa_test))):
            raise ValueError(
